...
func MinimalModuleTrapsAtInit() []byte {
	...
	types := sec(0x01, []byte{
		...
		0x60, 0x02, 0x7f, 0x7f, 0x00, // dealloc
		0x60, 0x00, 0x01, 0x7f, // supported_hooks
		0x60, 0x00, 0x01, 0x7e, // abi_version
	})
	funcs := sec(0x03, []byte{0x06, 0x00, 0x01, 0x02, 0x03, 0x05, 0x04})
	...
	exports = append(exports, append(name("dealloc"), 0x00, 0x03)...)
	exports = append(exports, append(name("abi_version"), 0x00, 0x04)...)
	exports = append(exports, append(name("supported_hooks"), 0x00, 0x05)...)
	...
	code := sec(0x0a, []byte{
		...
		0x02, 0x00, 0x0b, // dealloc
		0x08, 0x00, 0x42, 0x81, 0x80, 0x80, 0x80, 0x10, 0x0b, // abi
		0x04, 0x00, 0x41, byte(pb.Hook_HOOK_BEFORE_REQUEST.Bit()), 0x0b, // hooks
	})
...
func minimalModule(loopForever, withHooks bool) []byte {
	...
	// Types: alloc, run_hook, supported_hooks/abi_version, dealloc. run_hook takes TWO arguments: current ABI moved
	...
	if !withHooks {
		funcs = sec(0x03, []byte{0x04, 0x00, 0x01, 0x03, 0x04})
	}
	mem := sec(0x05, []byte{0x01, 0x00, 0x01})
	...
	deallocIndex, abiIndex := byte(3), byte(4)
	if !withHooks {
		deallocIndex, abiIndex = 2, 3
	}
	exports = append(exports, append(name("dealloc"), 0x00, deallocIndex)...)
	exports = append(exports, append(name("abi_version"), 0x00, abiIndex)...)
	...
	// dealloc: no-op; abi_version: packed (major=1, minor=1).
	codeBody = append(codeBody, 0x02, 0x00, 0x0b, 0x08, 0x00, 0x42, 0x81, 0x80, 0x80, 0x80, 0x10, 0x0b)
