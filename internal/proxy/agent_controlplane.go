...
var feedSchema = json.RawMessage(`{"type":"array","items":{"type":"object","required":["timestamp","provider","requested_model","status","latency_ms","tokens_in","tokens_out","cache_read_tokens","cache_write_tokens","bytes_in","bytes_out"],"properties":{"timestamp":{"type":"string"},"provider":{"type":"string"},"requested_model":{"type":"string"},"reported_model":{"type":"string"},"status":{"type":"integer"},"latency_ms":{"type":"number"},"tokens_in":{"type":"integer"},"tokens_out":{"type":"integer"},"cache_read_tokens":{"type":"integer"},"cache_write_tokens":{"type":"integer"},"bytes_in":{"type":"integer"},"bytes_out":{"type":"integer"}},"additionalProperties":true}}`)
...
func (s *Server) handlePluginAgentOperation(w http.ResponseWriter, r *http.Request) {
	...
	if expected := r.Header.Get("X-Torana-Plugin-Digest"); expected != "" {
		matches := false
		for _, loaded := range pipeline.AgentPlugins() {
			if loaded.Manifest.Name == pluginName && loaded.Digest == expected {
				matches = true
				break
			}
		}
		if !matches {
			writeAgentError(w, http.StatusPreconditionFailed, "stale_plugin_digest", "plugin changed; discover and review its current operation before retrying")
			return
		}
	}
